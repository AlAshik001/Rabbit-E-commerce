...
// Helper function to get a car by user Id or guest ID
...
router.post("/", async (req, res) =>{
    ...
    try {
        ...
        if(!protect) return res.status(404).json({message: "Product not Found"});
        
        // Determain if the user is logged in or guest
...
//@desc reomve a product from the cart
// @access public
    ...
    try {
        ...
        if(userCart){
          ...
          res.status(200).json(guestCart);
            ...
            // If the user has no exisiting  cart, assign the gust cart to the to user 
        ...
        if(userCart){
            // fuest cart has already been merged,return user cart
